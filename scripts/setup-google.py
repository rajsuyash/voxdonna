#!/usr/bin/env python3
"""Search Console work for any of the three properties: submit the sitemap, read the
numbers, check what Google has actually indexed. Optionally create a GA4 property.
...
Usage:
  ...
  python3 scripts/setup-google.py --report --site sc-domain:aisewak.com
  python3 scripts/setup-google.py --submit-sitemap --site sc-domain:aisewak.com \\
      --sitemap https://aisewak.com/sitemap.xml
"""
import argparse
import json
import subprocess
...
# One helper, three properties. --site/--sitemap override the defaults; the key
# pages Google is asked about follow whichever site is named, because inspecting
# voxdonna URLs against an aisewak property returns nothing and looks like a
# ranking problem rather than a wrong argument.
DEFAULT_SITE = "sc-domain:voxdonna.com"
DEFAULT_SITEMAP = "https://voxdonna.com/sitemap.xml"
KEY_PAGES = {
    "voxdonna.com": [
        "https://voxdonna.com/",
        "https://voxdonna.com/ai-voice-agents.html",
        "https://voxdonna.com/demos.html",
        "https://voxdonna.com/ai-for-manufacturers.html",
        "https://voxdonna.com/jewellers.html",
        "https://voxdonna.com/about.html",
        "https://voxdonna.com/blog/en/voice-ai-manufacturing-case-studies.html",
    ],
    "aisewak.com": [
        "https://aisewak.com/",
        "https://aisewak.com/ai-voice-agent-for-government",
        "https://aisewak.com/ai-voice-agent-for-elections",
        "https://aisewak.com/blog/multilingual-voice-ai-government-bhashini",
    ],
    "rajsuyash.com": [
        "https://rajsuyash.com/",
        "https://rajsuyash.com/about.html",
        "https://rajsuyash.com/blog/n8n-http-request-node-tutorial.html",
        "https://rajsuyash.com/blog/ai-voice-agent-pricing-cost-2026.html",
    ],
}


def host(site):
    """sc-domain:example.com and https://example.com/ both key on example.com."""
    return site.split(":", 1)[-1].replace("https://", "").replace("http://", "").strip("/")


def key_pages(site):
    return KEY_PAGES.get(host(site), [])
...
def submit_sitemap(hdrs, site, sitemap):
...
def report(hdrs, site):
    ...
    for dim in ("query", "page"):
        ...
        for r in resp.get("rows", []):
            label = r["keys"][0].replace(f"https://{host(site)}", "")[:52]
            print(f"    {label:54}{r['clicks']:>5.0f} clicks {r['impressions']:>6.0f} impr  pos {r['position']:.1f}")
        if not resp.get("rows"):
            print("    none")

    if not key_pages(site):
        print(f"\n  no key pages configured for {host(site)} — add them to KEY_PAGES")
        return
    print("\n  index status of key pages")
    for u in key_pages(site):
        ...
        label = u.replace(f"https://{host(site)}", "") or "/"
        if r:
...
def main():
    ...
    ap.add_argument("--sitemap", default=None,
                    help="sitemap URL (default: the named site's /sitemap.xml)")
    a = ap.parse_args()
    sitemap = a.sitemap or (DEFAULT_SITEMAP if a.site == DEFAULT_SITE
                            else f"https://{host(a.site)}/sitemap.xml")
    if not any((a.report, a.submit_sitemap, a.ga)):
        ap.error("pick at least one of --report, --submit-sitemap, --ga")
    hdrs = auth()
    if a.submit_sitemap:
        submit_sitemap(hdrs, a.site, sitemap)
    if a.report:
        report(hdrs, a.site)
    if a.ga:
        ga(hdrs)
