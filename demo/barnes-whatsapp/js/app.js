/*
 * Camille — Concierge immobilier WhatsApp BARNES · contrôleur de démo interactif.
...
(function () {
    ...
    c.querySelectorAll('.pbtn').forEach(btn => btn.addEventListener('click', () => {
      ...
      botSay([
        { t:'text', text: btn.dataset.act === 'reserve'
            ? `C'est fait ! ✨ J'ai bloqué le *${p2.name}* (${eur(p2.price)}) pendant 7 jours avec un dépôt de garantie remboursable. Votre conseiller vous envoie le contrat de réservation.`
  ...
  const RENDER = {
    ...
    rate() {
      ...
      const n = el('div', 'ratecard', `
        ...
        <div class="rc-row"><span>Paris (prestige)</span><b>${eur(r.m2['Paris'])}/m²</b></div>
        <div class="rc-row"><span>Côte d'Azur</span><b>${eur(r.m2["Côte d'Azur"])}/m²</b></div>
        <div class="rc-row"><span>Alpes (Courchevel)</span><b>${eur(r.m2['Alpes'])}/m²</b></div>
    ...
    appt(spec) {
      ...
      if (spec.kind === 'reserve') {
        ...
        title = '✅ Lot réservé'; body = `${p.name}<br><b>${eur(p.price)}</b> · dépôt remboursable · 7 jours<br>Réf : <b>RES${(1000+(state.leadCount*7)%9000)}</b>`;
        ...
        title = '📅 Visite confirmée'; body = `${window.STORE_BY_CITY[spec.store.toLowerCase()].name}<br><b>${spec.when}</b><br>Conseiller assigné · visite du bien + secteur`;
      }
    ...
    voiceOut(spec) { renderVoice(spec, 'in'); },   // réponse vocale de Camille (côté entrant)
    rag(spec) {
  ...
  function renderVoice(spec, side) {
    ...
    b.innerHTML = `
      ...
      <div class="vc-tx"><b>${isIn ? 'Camille' : 'Reçu'} (${spec.lang}) :</b> ${spec.text}<br><i>« ${spec.gloss} »</i></div>
  ...
  async function handleFreeText(text) {
    ...
    // 4) visite / bureau
    else if (/(visite|visiter|rendez-vous|rdv|bureau|boutique|proche|adresse|itinéraire|itineraire)/.test(t)) {
      await botSay([{ t:'text', text:'Nos conseillers reçoivent tous les jours à *Paris*, *Saint-Tropez*, *Courchevel* et *Aix* — dites-moi le secteur et je réserve un créneau. En voici un :' }, { t:'store', city:"Côte d'Azur" }]);
    }
    // 5) salutation / repli
    else if (/(bonjour|bonsoir|salut|coucou|hello|hey)/.test(t)) {
      await botSay([{ t:'text', text:'Bonjour ! 🙏 Je suis *Camille*, la concierge immobilière BARNES. Je peux *trouver votre bien*, envoyer *dossiers & fiscalité*, calculer un *financement*, *suivre un chantier* ou *organiser une visite privée*. Que puis-je pour vous ?' }]);
    }
    else {
      await botSay([{ t:'text', text:'Avec plaisir ! 🏡 Je suis à l\'aise sur la recherche de biens, l\'échéancier VEFA, le financement, les frais de notaire & la fiscalité, le suivi de chantier et les visites. Essayez « villa vue mer à Saint-Tropez, 9 M€ », ou choisissez un scénario dans le menu à gauche. Un conseiller reste à un message.' }]);
    }
  ...
  function detectConfig(t) {
    ...
    if (/hôtel particulier|hotel particulier/.test(t)) return 'Hôtel particulier';
    if (/(mas|domaine|vignoble)/.test(t)) return 'Mas';
    if (/studio/.test(t)) return 'Studio';
    return null;
  }
  function detectCity(t) {
    if (/paris|marais|rive gauche|rive droite|haussmann/.test(t)) return 'Paris';
    if (/saint-tropez|st-tropez|cannes|nice|cap-ferrat|côte d'azur|cote d'azur|riviera|azur|ramatuelle/.test(t)) return "Côte d'Azur";
    if (/courchevel|megève|megeve|alpes|chamonix|ski|montagne/.test(t)) return 'Alpes';
    if (/luberon|provence|aix|bordeaux|saint-émilion|saint-emilion|vignoble/.test(t)) return 'Provence';
    return null;
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    botSay([
      { t:'text', text:'👋 Bienvenue ! Je suis *Camille*, la concierge immobilière IA de *BARNES*. Je suis une assistante IA — et un conseiller reste toujours à un message.' },
      { t:'text', text:'Choisissez un scénario à gauche, tapez une réponse rapide, ou *écrivez* simplement ce que vous cherchez — par ex. « villa vue mer à Saint-Tropez, 9 M€ ». 🏡' },
    ]).then(() => {
  ...
  window.CamilleDemo = { startScenario, state };
})();
