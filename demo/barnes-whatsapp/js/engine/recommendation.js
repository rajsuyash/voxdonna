...
(function () {
  ...
  function reason(p, q) {
    const bits = [];
    if (q.budget) {
      if (p.price <= q.budget) bits.push(`dans votre budget de ${(q.budget/1000000).toFixed(1)} M€`);
    ...
    const audLabel = { 'residence-secondaire':'résidence secondaire', 'residence-principale':'résidence principale',
      'investissement':'investissement', 'pied-a-terre':'pied-à-terre', 'international':'acquéreur international', 'prestige':'prestige' };
