...
function limited(string $key, int $max, int $window): bool {
    $file = sys_get_temp_dir() . '/luke_diamond_' . md5($key);
    $now = time();
...
function confirm_booking(array $body, string $eventId, callable $dm, ?string $directory = null): array {
    ...
    $directory ??= dirname(__DIR__, 3) . '/.luke-diamond-bookings';
    ...
    try {
        ...
        if (isset($state['result'])) {
            ...
            $fields = ['firstName' => $name, 'channel' => 'whatsapp_web', 'custom_fields' => ['luke_diamond_store' => $slot['store']['name'], 'luke_diamond_visit' => $slot['label']]];
        ...
        error_log('Luke Diamond booking failed: ' . get_class($error));
        return [503, ['error' => 'Booking could not finish. Retry the same visit to check its saved status.']];
    } finally { flock($lock, LOCK_UN); fclose($lock); }
}

if (PHP_SAPI === 'cli' && defined('LUKE_DIAMOND_TEST')) return;
...
if ($route === 'agent/book') {
    ...
    // Same derived token as the sibling personal-shopper demo: one DM Champ key backs both,
    // so a separate salt would buy no isolation. Give this demo its own salt the day it gets its own key.
